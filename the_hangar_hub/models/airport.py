...
class PostGroup:
    ...
    def num_replies(self):
        direct_replies = len([x for x in self.replies if not x.deleted()])
        indirect_replies = 0

        if self.replies:
            for reply in [x for x in self.replies if not x.deleted()]:
                d, t = reply.num_replies()
                indirect_replies += t
    ...
    def deleted(self):
        ...
        (This post and any replies will no longer be displayed)
        """
        return self.post.deleted

    def flagged(self):
        """
        ToDo: I should remove this and require is_flagged/was_flagged
        Has THIS post been flagged?
        True even if deleted
        (does not look at replies)
        """
        if self.post.flagged:
            # If a flagged post has been reviewed, it is no longer considered flagged
            # (the flag remains to prevent future re-flagging)
            return not self.post.reviewed
        return False

    def is_flagged(self):
        """
        Has THIS post been flagged?
        Not true if flagged and then deleted or reviewed
        (does not look at replies)
        """
        return self.post.is_flagged()

    def was_flagged(self):
        """
        Has THIS post ever been flagged?
        """
        return self.post.flagged

    def can_flag(self):
        return self.post.can_flag()

    def flagged_or_deleted(self):
        return self.flagged() or self.deleted()

    def contains_flag(self):
        """
        ...
        if self.deleted():
            return False
        if self.replies:
            if [x for x in self.replies if x.is_flagged()]:
                return True
            if [x for x in self.replies if x.contains_flag()]:
                return True
        return False
...
class MessageBoardEntry(models.Model):
    ...
    def is_flagged(self):
        """
        Flagged without completed review (approved or deleted)
        """
        if self.flagged:
            return not (self.reviewed or self.deleted)
        return False

    def was_flagged(self):
        """
        Was ever flagged (may have been reviewed or deleted)
        """
        return self.flagged

    def can_flag(self):
        log.debug(f"{self.id} can be flagged: {not (self.was_flagged() or self.reviewed)}")
        return not (self.was_flagged() or self.reviewed)
